...
PC_def_update.pl ontologies/fbcv-edit.obo > tmp.obo # Pull defs from named equivalent class to FBcv (from EC CHEBI terms)
...
owltools --catalog-xml ontologies/catalog-v001.xml ontologies/lethal_phase-edit.owl --merge-import-closure -o file://`pwd`/lethal_phase_imports_merged.owl
echo 'owltools tmp2.obo --merge lethal_phase_imports_merged.owl --merge fbcv_auth_attrib_licence.owl -o file://`pwd`/fbcv_merged.owl' 2>&1

owltools tmp2.obo --merge lethal_phase_imports_merged.owl --merge ontologies/fbcv_auth_attrib_licence.owl -o file://`pwd`/fbcv_merged.owl
